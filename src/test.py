...
def load_trained_model(
    ...
    """
    Загружает обученную модель и связанную с ней информацию со словарём.
    """
...
def generate_completion(
    ...
    """
    Генерирует продолжение для введённой пользователем фразы.

    :param prompt: исходная фраза пользователя
    :param max_new_tokens: максимальное количество предсказываемых токенов
    """
    pad_idx = stoi.get("<pad>", 0)
    unk_idx = stoi.get("<unk>", 1)

    # Простая токенизация по пробелам; должна быть согласована с обучением
